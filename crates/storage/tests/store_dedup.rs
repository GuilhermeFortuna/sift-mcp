...
#[test]
fn batch_dedupes_identical_content_hashes() {
    let dir = tempdir().unwrap();
    let mut store = ChunkStore::create(dir.path(), 4, "m").unwrap();
    let hash = [7u8; 32];
    let r1 = record_with_hash(hash, "a");
    let r2 = record_with_hash(hash, "b"); // same hash, different metadata — hash wins
...
#[test]
fn insert_reuses_existing_live_hash() {
    let dir = tempdir().unwrap();
    let mut store = ChunkStore::create(dir.path(), 4, "m").unwrap();
    let hash = [9u8; 32];
